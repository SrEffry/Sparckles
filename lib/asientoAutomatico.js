 ...
 *
 * El documento se NORMALIZA con la misma función que usa `RetencionPracticada`, y no se guarda
 * crudo. Antes se escribía tal cual venía escrito en el documento, así que `900123456`,
 * `900.123.456` y `900123456-7` quedaban como TRES terceros distintos en el libro auxiliar.
 *
 * Eso importa más de lo que parece: la información exógena agrupa el año entero por tercero y
 * la DIAN cruza el 1001 del pagador contra el 1007 del receptor. Un mismo proveedor partido en
 * tres es un cruce que no cuadra, y el art. 651 E.T. sanciona la información errónea igual que
 * la que falta. Es la misma incoherencia que `lib/retencionesPracticadas.js` ya advertía que
 * rompía los certificados.
 *
 * Si el documento no trae dígitos utilizables se cae al nombre, que es mejor que un nulo: el
 * tablero de preparación de Reportes los lista para que alguien los complete.
 */
const t3 = (nombre, doc) => normalizarDocumento(doc).numero || nombre || null;
...
//
// El asiento cubre los TRES bloques del costo laboral: el devengo del trabajador, los aportes
// patronales (salud, pensión, ARL, parafiscales) y las prestaciones sociales (cesantías,
// intereses, prima, vacaciones). Cada uno va a su propia cuenta de gasto y a su propio pasivo,
// porque se pagan a acreedores distintos y en fechas distintas.
//
// LO QUE NO HACE: no genera la planilla PILA ni la nómina electrónica (Res. DIAN 000013/2021),
// y no practica retención en la fuente sobre rentas de trabajo (art. 383 E.T.).
export function movimientosDeNomina(n, mapa) {
  const a = crearAsiento();
  const tercero = t3(n.empleadoNombre, n.empleadoDocumento);

  // ── Gasto: los tres bloques del costo laboral ──
  // El AUXILIO DE TRANSPORTE va aparte: no es salario (art. 128 CST) y la nómina electrónica lo
  // exige identificado. Mezclarlo con el sueldo es el mismo defecto que se corrigió del lado
  // patronal —una cuenta llamada "Salario Básico" recibiendo cosas que no son salario—.
  a.debito(mapa.gastoNomina, Number(n.totalDevengos) - Number(n.transporte || 0), {
    concepto: "Gasto de nómina",
    tercero,
  });
  a.debito(mapa.gastoAuxilioTransporte, n.transporte, { concepto: "Auxilio de transporte", tercero });
  // Cada componente a su propia cuenta de gasto, igual que el pasivo: el estado de resultados
  // tiene que poder responder cuánto costó la ARL sin abrir el libro auxiliar.
  a.debito(mapa.gastoSaludPatronal, n.saludPatronal, { concepto: "Salud a cargo del empleador", tercero });
  a.debito(mapa.gastoPensionPatronal, n.pensionPatronal, { concepto: "Pensión a cargo del empleador", tercero });
  a.debito(mapa.gastoArl, n.arl, { concepto: "Riesgos laborales (ARL)", tercero });
  a.debito(mapa.gastoParafiscales, Number(n.sena) + Number(n.icbf) + Number(n.cajaCompensacion), {
    concepto: "Aportes parafiscales (SENA, ICBF, caja)",
    tercero,
  });
  a.debito(mapa.gastoCesantias, n.cesantias, { concepto: "Cesantías", tercero });
  a.debito(mapa.gastoInteresesCesantias, n.interesesCesantias, {
    concepto: "Intereses sobre cesantías",
    tercero,
  });
  a.debito(mapa.gastoPrima, n.prima, { concepto: "Prima de servicios", tercero });
  a.debito(mapa.gastoVacaciones, n.vacaciones, { concepto: "Vacaciones", tercero });

  // ── Seguridad social: se consigna la suma de lo del trabajador y lo del empleador ──
  a.credito(mapa.saludPorPagar, Number(n.salud) + Number(n.saludPatronal), {
    concepto: "Aportes de salud por pagar",
    tercero,
  });
  // El fondo de solidaridad pensional se consigna junto con la pensión.
  a.credito(mapa.pensionPorPagar, Number(n.pension) + Number(n.pensionPatronal) + Number(n.fsp || 0), {
    concepto: "Aportes de pensión por pagar",
    tercero,
  });
  a.credito(mapa.arlPorPagar, n.arl, { concepto: "Aportes de ARL por pagar", tercero });
  a.credito(
    mapa.parafiscalesPorPagar,
    Number(n.sena) + Number(n.icbf) + Number(n.cajaCompensacion),
    { concepto: "Aportes parafiscales por pagar", tercero }
  );

  // ── Prestaciones: cada una a su propia cuenta, porque se pagan en momentos distintos ──
  a.credito(mapa.cesantiasPorPagar, n.cesantias, { concepto: "Cesantías por pagar", tercero });
  a.credito(mapa.interesesCesantiasPorPagar, n.interesesCesantias, {
    concepto: "Intereses sobre cesantías por pagar",
    tercero,
  });
  a.credito(mapa.primaPorPagar, n.prima, { concepto: "Prima de servicios por pagar", tercero });
  a.credito(mapa.vacacionesPorPagar, n.vacaciones, { concepto: "Vacaciones por pagar", tercero });

  // ── Lo que queda por entregarle al trabajador ──
...
export async function contabilizarDocumento(tx, { usuarioId, tipo, documento, mapa, descripcion, docRef, fecha }) {
  ...
  // No se duplica el prefijo cuando la referencia ya lo trae: el documento soporte se numera
  // "DS-2026-0038" y anteponerle otro "DS-" dejaba en el libro "DS-DS-2026-0038".
  const base = String(docRef).startsWith(`${meta.prefijo}-`) ? String(docRef) : `${meta.prefijo}-${docRef}`;
  const numero = await numeroLibre(tx, usuarioId, base);
...
const DATOS_LIBRO = {
  ...
  nomina: (n) => ({
    // El periodo y el documento del trabajador, no un trozo del id: `NM-2026-03-1098765432` se
    // lee en el libro sin abrir el auxiliar, `NM-A1B2C3` no dice nada.
    docRef: `${n.periodo}-${n.empleadoDocumento || n.id.slice(-6).toUpperCase()}`,
