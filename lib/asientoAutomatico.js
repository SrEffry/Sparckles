...
export function movimientosDeFactura(f, mapa) {
...
export function movimientosDeCompra(c, mapa) {
  const a = crearAsiento();
...
export function movimientosDeSoporte(s, mapa) {
  ...
  a.credito(mapa.proveedores, s.neto, { concepto: "Proveedores", tercero });
...
export async function contabilizarDocumento(tx, { usuarioId, tipo, documento, mapa, descripcion, docRef, fecha }) {
  ...
  const { movimientos, totalDebitos, totalCreditos, diferencia, faltantes } = generador(documento, mapa);
  ...
  // descuadre se arrastra y nadie sabe de dónde salió.
  if (diferencia > 0.01) {
    return { faltantes: [`El asiento propuesto no cuadra por ${diferencia}. Revisa las cuentas del mapa.`] };
  }
...
export async function contabilizarYEnlazar(tx, { usuarioId, tipo, documento, mapa }) {
  ...
  if (!resultado.asiento) return resultado;

  await tx[MODELO[tipo]].update({
    where: { id: documento.id },
    data: { asientoId: resultado.asiento.id },
  });
  return resultado;
}
...
export async function reversarAsientoDe(tx, { usuarioId, asientoId, fecha, motivo }) {
  ...
  // Si YA tiene contraasiento, no se reversa dos veces: dejaría el saldo invertido. Se
  // comprueba por `documentoRef` + tipo y no por el número, porque el número puede llevar
  // sufijo y lo que importa es que ese asiento concreto ya esté anulado.
  const yaReversado = await tx.asiento.findFirst({
    where: { usuarioId, descripcion: { startsWith: `Reversión de ${original.numero}` } },
    select: { id: true },
  });
