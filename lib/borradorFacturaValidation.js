...
export function validarBorradorFactura(body) {
  ...
  items.forEach((it, i) => {
    ...
    // PRECIO PACTADO. Se guarda porque ahora `calcularFactura` SÍ lo honra: el del catálogo es
    // solo el valor por defecto. Vacío = usar el del catálogo al emitir, que es lo que conviene
    // en un borrador viejo de un producto cuyo precio de lista cambió a propósito.
    //
    // Negativo no: una devolución se hace con nota crédito, no facturando en negativo.
    const precio =
