...
export function validarBorradorFactura(body) {
  ...
  items.forEach((it, i) => {
    ...
    const vacia =
      !it.productoId && (it.cantidad == null || it.cantidad === "") &&
      (it.precioUnitario == null || it.precioUnitario === "");
    if (vacia) return;
    ...
    // El precio puede venir vacío: al emitir lo pone el catálogo. Lo que no puede es ser
    // negativo, que no significa nada en una factura de venta.
    const precio =
