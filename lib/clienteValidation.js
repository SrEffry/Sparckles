...
export function normalizarCliente(body) {
  ...
  // Documento NORMALIZADO, que es la clave de agrupación de la exógena y la que sostiene el
  // `@@unique`. Se calcula aquí y no en cada consulta: si se derivara al vuelo, dos fichas del
  // mismo NIT escritas distinto entrarían sin chocar y saldrían como dos terceros.
  data.documentoNormalizado = normalizarDocumento(data.nit || data.numeroDocumento).numero;
  data.activo = body.activo !== undefined ? !!body.activo : true;
