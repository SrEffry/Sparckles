...
import { buscarConceptoComprobante } from "@/lib/conceptosComprobante";
...
export function normalizarComprobante(body, mapa = null) {
  const errors = [];
  ...
  // MODO. 'aplicacion' cancela el saldo de un documento previo; 'imputacion' registra un
  // movimiento que no tiene documento (nómina, impuestos, anticipos, caja menor). Sin el
  // segundo modo esos pagos no tenían por dónde registrarse en ningún módulo.
  const modo = body.modo === "imputacion" ? "imputacion" : "aplicacion";
  ...
  // IMPUTACIONES. El usuario elige el CONCEPTO; la cuenta la resuelve el mapa. Nunca se acepta
  // un código PUC del cliente: eso convertiría el comprobante en un asiento manual disfrazado.
  const imputaciones = [];
  if (modo === "imputacion") {
    const brutas = Array.isArray(body.imputaciones) ? body.imputaciones : [];
    const conMonto = brutas.filter((i) => Number(i.valor) > 0);
    if (conMonto.length === 0) errors.push("Agrega al menos un concepto con valor.");

    for (const [i, linea] of conMonto.entries()) {
      const concepto = buscarConceptoComprobante(tipo, T(linea.concepto));
      if (!concepto) {
        errors.push(`Línea ${i + 1}: elige un concepto válido.`);
        continue;
      }
      const cuentaPuc = T(mapa?.[concepto.cuenta]);
      if (!cuentaPuc) {
        errors.push(
          `El concepto "${concepto.etiqueta}" necesita la cuenta "${concepto.cuenta}" en el mapa de cuentas.`
        );
        continue;
      }

      // Cartera y proveedores tienen saldo materializado por otros módulos. Moverlos por
      // imputación libre desincronizaría el libro contra los pendientes por cobrar y por
      // pagar — es justo lo que el blindaje de las notas de contabilidad impide.
      if (cuentaPuc === T(mapa?.clientes) || cuentaPuc === T(mapa?.proveedores)) {
        errors.push(
          `El concepto "${concepto.etiqueta}" apunta a una cuenta de cartera o proveedores, que solo se mueve aplicando el comprobante a una factura o a una compra.`
        );
        continue;
      }

      // Art. 771-2 E.T.: un pago por bienes o servicios sin factura ni documento soporte no
      // es deducible. Si el concepto es de naturaleza costo o gasto, se exige la referencia.
      const soporteRef = T(linea.documentoSoporteRef);
      if (concepto.requiereSoporte && !soporteRef) {
        errors.push(
          `"${concepto.etiqueta}" necesita el número de la factura o del documento soporte: sin él el gasto no es deducible (Art. 771-2 E.T.).`
        );
        continue;
      }
      ...
      imputaciones.push({
        ...
        cuentaPuc,
        valor: r2(linea.valor),
        detalle: T(linea.detalle) || null,
        documentoSoporteRef: soporteRef || null,
      });
    }
  }
