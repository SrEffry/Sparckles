...
export async function liquidarFactura({ usuarioId, entrada }) {
  ...
  // Una factura con TODAS las líneas en cero no es una venta: es un documento que consumiría un
  // consecutivo DIAN sin operación detrás. Una línea suelta en cero sí es legítima (bonificación
  // en especie, muestra) y solo se avisa.
  if (calc.todoEnCero) throw new ErrorEmision("TODO_EN_CERO");
...
export async function emitirFactura({ usuarioId, entrada, alEmitir }) {
  ...
  return {
    ...
    contabilizacion,
    avisos: [...(avisoVigencia ? [avisoVigencia] : []), ...(calc.avisosPrecio || [])],
  };
}
...
export function errorDeEmision(e) {
  switch (e?.code) {
    ...
    case "TODO_EN_CERO":
      return {
        error:
          "Todas las líneas van en cero: eso no es una venta y consumiría un número de la resolución. Si es una entrega gratuita, revisa el retiro de inventarios (art. 421 lit. b E.T.).",
        status: 400,
      };
    case "INSTRUMENTOS":
      return {
