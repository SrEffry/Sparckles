...
export function normalizarEmpleado(body) {
  ...
  if (salarioBase > 0 && salarioBase < smlmv) {
    avisos.push(
      `El salario está por debajo del mínimo legal de ${anio} (${smlmv.toLocaleString("es-CO")}). Solo es válido si la jornada es parcial; aun así, los aportes se cotizarán sobre el mínimo.`
    );
  }
