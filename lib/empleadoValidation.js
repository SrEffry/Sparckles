...
export function normalizarEmpleado(body) {
  ...
  const data = {
    ...
    // ---- Datos para el formato 2276 de exógena (rentas de trabajo) ----
    // Opcionales: no pueden impedir dar de alta a un empleado hoy por un dato que solo hace
    // falta al reportar. Lo que quede vacío sale en el tablero de preparación de Reportes.
    // Los cuatro campos de nombre van SEPARADOS porque así los pide la DIAN; no se deducen
    // partiendo `nombres` y `apellidos`, que admiten cualquier cosa dentro.
    tipoDocumentoDian: CODIGOS_DOCUMENTO_DIAN.has(T(body.tipoDocumentoDian))
      ? T(body.tipoDocumentoDian)
      : null,
    primerApellido: T(body.primerApellido) || null,
    segundoApellido: T(body.segundoApellido) || null,
    primerNombre: T(body.primerNombre) || null,
    otrosNombres: T(body.otrosNombres) || null,
    direccion: T(body.direccion) || null,
    codigoDepartamento: T(body.codigoDepartamento) || null,
    codigoMunicipio: T(body.codigoMunicipio) || null,
    codigoPais: T(body.codigoPais) || null,
  };
