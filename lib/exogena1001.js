...
export async function generar1001(usuarioId, anio) {
  ...
  // El sector decide el mapa cuenta→concepto: los dos catálogos difieren en cuentas clave.
  const mapaCuentas = await prisma.mapaCuentas.findUnique({ where: { usuarioId } });
  const sector = mapaCuentas?.sector === "esal" ? "esal" : "comercial";
  ...
  let sinClasificar = 0;
  let sinTercero = 0;
  let movimientosDeNomina = 0;
  let terceroSinDocumento = 0;
  let filasNegativas = 0;
  let valorNegativo = 0;

  for (const m of movimientos) {
    if (!cuentaReportable(m.cuenta)) continue;

    // NÓMINA FUERA. Los pagos por rentas de trabajo van al formato 2276, no al 1001; y los
    // aportes patronales van al 1001 pero en cabeza de la EPS, la caja o el fondo, no del
    // empleado — y el asiento de nómina pone el documento del EMPLEADO en todas sus líneas.
    // Emitirlos hoy reportaría la nómina del año contra las cédulas de los trabajadores.
    if (esCuentaDeNomina(m.cuenta)) {
      movimientosDeNomina += 1;
      continue;
    }

    // UN NOMBRE NO ES UN DOCUMENTO. `asientoAutomatico` cae al nombre cuando el tercero no
    // tiene documento, y ese texto acabaría impreso en la casilla "Número identificación".
    if (!/^\d+$/.test(m.tercero)) {
      terceroSinDocumento += 1;
      continue;
    }
    ...
    const concepto = concepto1001DeCuenta(m.cuenta, sector);
  ...
  for (const r of retenciones) {
    ...
    if (suyas.length === 1) {
      ...
      // Antes solo contaba cuando el tercero no tenía NINGUNA fila; el caso de VARIAS
      // filas sin coincidencia de concepto creaba una fila con retención y pago 0 sin
      // avisarlo, que es justo la inconsistencia que el cruce contra renta destapa.
      if (!exacta) retSinFila += 1;
    }
  ...
  const avisos = [
    ...
    'Las columnas "Retención en la fuente asumida Renta" y "Retención practicada IVA a no residentes" van en CERO: el sistema no registra ninguno de esos dos hechos.',
    "NO se incluye la NÓMINA. Los sueldos, prestaciones y auxilios son rentas de trabajo y van al formato 2276, que este sistema NO genera. Y los aportes patronales (conceptos 5010, 5011 y 5012) van al 1001 pero en cabeza de la EPS, la caja o el fondo — y hoy no se guarda el NIT de esas entidades, así que saldrían contra la cédula del empleado. Hay que reportarlos aparte.",
    "El concepto 5008 (compra de ACTIVOS FIJOS) no se produce todavía: toda compra debita la misma cuenta de inventario, así que no hay con qué separarlo del 5007. `CompraItem.esActivoFijo` existe pero el asiento aún no lo usa.",
  ];
  if (movimientosDeNomina)
    avisos.push(
      `Se excluyeron ${movimientosDeNomina} movimiento(s) de nómina, por lo dicho arriba. Si la empresa tiene empleados, su reporte de rentas de trabajo hay que hacerlo por fuera.`
    );
  if (terceroSinDocumento)
    avisos.push(
      `${terceroSinDocumento} movimiento(s) quedaron FUERA porque su tercero está identificado por NOMBRE y no por documento. La exógena identifica por el par (tipo, número): un nombre en la casilla del NIT hace rechazar la fila.`
    );
  if (filasNegativas)
    avisos.push(
      `⚠️ ${filasNegativas} fila(s) daban un pago NEGATIVO por ${Math.abs(valorNegativo).toLocaleString("es-CO")} y se excluyeron. Suele ser la anulación de un documento de un año anterior, cuyo contraasiento se fecha hoy: eso se reporta en el concepto 5028 ("devolución de pagos de años anteriores") en POSITIVO, y hay que agregarlo a mano.`
    );
  if (sinClasificar)
    avisos.push(
  ...
  // NUNCA SE IMPRIME UN NEGATIVO. Un neto negativo aparece cuando el contraasiento de una
  // anulación cae en un año posterior al documento (se fecha HOY, no en la fecha original): el
  // AG anterior ya reportó el pago y este año queda el reverso. El prevalidador rechaza importes
  // negativos, y el concepto previsto para eso es el 5028 ("devolución de pagos o abonos en
  // cuenta de años anteriores"), que se reporta en POSITIVO. Como el sistema no puede distinguir
  // con certeza esa causa de un error de registro, se EXCLUYE la fila y se avisa con su valor
  // en vez de reclasificarla a ciegas.
  for (const f of acc.values()) {
    if (f.deducible < 0) {
      filasNegativas += 1;
      valorNegativo += f.deducible;
    }
  }
  ...
  const filas = [...acc.values()]
    ...
    .map((f) => ({
      ...f,
      deducible: n2(Math.max(0, f.deducible)),
      noDeducible: 0,
      ivaMayorValorDeducible: 0,
      ivaMayorValorNoDeducible: 0,
