...
export async function diagnosticoExogena(usuarioId, anioGravable) {
  ...
  //
  // `Empleado` SÍ guarda los campos del 2276 —tipo de documento DIAN, los cuatro nombres,
  // dirección y códigos DANE— desde la fase 0. Este bloque seguía empujando "Sin tipo de
  // documento DIAN" INCONDICIONALMENTE, con un comentario que decía que el modelo no los
  // guardaba: una empresa con 40 empleados completos veía 40 críticos permanentes y el tablero
  // —que es la única herramienta para arreglar los datos a tiempo— perdía toda su señal.
  //
  // Se evalúan con `pendientesDeTercero`, la misma regla que clientes y proveedores: un empleado
  // es siempre persona natural, así que se exigen los nombres separados y no una razón social.
  const empleadosRevisados = empleados.map((e) => {
    const doc = normalizarDocumento(e.documento);
    const nombre =
      [e.nombres, e.apellidos].filter(Boolean).join(" ").trim() || "(sin nombre)";
    const { criticos, faltantes } = pendientesDeTercero({
      tipo: "natural",
      documento: doc.numero,
      tipoDocumentoDian: e.tipoDocumentoDian,
