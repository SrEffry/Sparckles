...
export async function diagnosticoExogena(usuarioId, anioGravable) {
  ...
  const [clientes, compras, soportes, empleados] = await Promise.all([
    ...
    prisma.compra.findMany({
      ...
      select: { proveedorNombre: true, proveedorNit: true },
    }),
  ...
  const clientesRevisados = clientes.map((c) => {
    ...
    const r = revisarTercero({
      ...
      tipoDocumentoDian: c.tipoDocumentoDian,
      numeroDocumento: esNatural ? c.numeroDocumento : c.nit,
    ...
    return { id: c.id, origen: "cliente", nombre: c.nombreCompleto, tipo: c.tipo, ...r };
  });

  // ---- Proveedores derivados (todavía no hay tabla) ----
  const porDoc = new Map();
  const registrar = (nombre, documento, tipoDoc) => {
  ...
  compras.forEach((c) => registrar(c.proveedorNombre, c.proveedorNit, null));
  soportes.forEach((s) =>
    registrar(s.proveedorNombre, s.proveedorDocumento, s.proveedorTipoDocumento)
  );

  const proveedores = [...porDoc.values()].map((p) => {
    const criticos = [];
    if (!p.documento) criticos.push("Sin número de identificación");
    if (!p.tipoDoc || !codigoDianDesdeTexto(p.tipoDoc)) criticos.push("Sin tipo de documento DIAN");

    return {
      id: p.clave,
      origen: "proveedor",
      nombre: [...p.nombres][0] || "(sin nombre)",
      documento: p.documento,
      documentos: p.documentos,
      criticos,
      // Un proveedor derivado nunca tiene dirección ni códigos: no hay dónde guardarlos.
      faltantes: ["Dirección y códigos DANE (no hay tabla de proveedores)"],
