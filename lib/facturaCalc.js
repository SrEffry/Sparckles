...
/**
 * Tratamiento de IVA del producto, que la tarifa numérica por sí sola no distingue.
 *
 * Exento (Art. 477 E.T.) y excluido (Art. 476) liquidan ambos en 0 pero no son lo mismo: el
 * exento es gravado a tarifa 0% y da derecho a IVA descontable y devolución; el excluido no
 * genera ni da derecho, y su IVA se lleva al costo. Van en renglones distintos de la
 * declaración, y confundirlos rompe el prorrateo del descontable (Art. 490).
 *
 * "0%" no es una categoría del régimen colombiano: es una opción heredada del formulario de
 * producto. Se marca 'sin_clasificar' en vez de suponer, para que se pueda corregir.
 */
export function tipoIvaDelProducto(tarifaIva) {
  const s = String(tarifaIva || "").trim().toLowerCase();
  if (s === "exento") return "exento";
  if (s === "excluido") return "excluido";
  if (s === "0%" || s === "0" || s === "") return "sin_clasificar";
  return "gravado";
}
...
export function calcularFactura({
  ...
  for (const it of items) {
    ...
    // Si el emisor no es responsable, el renglón no pertenece a ninguna categoría de IVA:
    // no declara IVA. El tratamiento propio del producto se conserva en `extra`.
    const tipoProducto = tipoIvaDelProducto(p.tarifaIva);
