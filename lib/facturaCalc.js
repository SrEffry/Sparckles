...
import { tarifaNumerica, tratamientoIva } from "@/lib/data/tarifasIva";

// La tabla de tarifas y su interpretación viven en `lib/data/tarifasIva.js`, que es la
// fuente única: la UI, la validación del servidor y el importador leen de ahí.
// Se reexportan con los nombres previos para no romper a quien ya las importaba.
export { tarifaNumerica as parseTarifaIva, tratamientoIva as tipoIvaDelProducto };
...
export function calcularFactura({
  ...
  for (const it of items) {
    ...
    const tarifaProducto = tarifaNumerica(p.tarifaIva);
    ...
    const tipoProducto = tratamientoIva(p.tarifaIva);
