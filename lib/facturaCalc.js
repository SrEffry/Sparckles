...
export function calcularFactura({
  ...
  const lineas = [];
  // Avisos sobre el PRECIO PACTADO (art. 90 E.T. y facturación en cero). Van al llamador para
  // que la pantalla los muestre al emitir: no bloquean nada.
  const avisosPrecio = [];
  let lineasEnCero = 0;
  ...
  for (const it of items) {
    ...
    // ART. 90 E.T.: el valor pactado se presume, pero puede cuestionarse si se aparta
    // NOTORIAMENTE del comercial promedio — y la norma fija ese "notoriamente" en más del 15%.
    // Se AVISA y no se bloquea, a propósito: el precio de lista del catálogo NO es "el precio
    // comercial promedio del mercado", así que el software no puede afirmar que hay infracción.
    // Una liquidación de temporada al 40% es perfectamente legítima. Lo que sí puede hacer es
    // dejar constancia de que alguien lo vio. Al alza no se avisa: el riesgo del art. 90 es
    // vender barato, no caro.
    const desviacion =
      precioCatalogo > 0 && precioUnitario < precioCatalogo
        ? (precioCatalogo - precioUnitario) / precioCatalogo
        : 0;
    if (desviacion > 0.15) {
      avisosPrecio.push(
        `${p.descripcion}: se factura a ${precioUnitario.toLocaleString("es-CO")} frente a un precio de lista de ${precioCatalogo.toLocaleString("es-CO")} (${Math.round(desviacion * 100)}% por debajo). El art. 90 E.T. permite cuestionar un precio que se aparte más del 15% del comercial promedio.`
      );
    }
    if (precioUnitario === 0) {
      lineasEnCero += 1;
      avisosPrecio.push(
        `${p.descripcion}: se factura en CERO. Una entrega a título gratuito es un retiro de inventarios (art. 421 lit. b E.T.) que causa IVA sobre el valor comercial del bien, y el sistema no lo liquida.`
      );
    }
  ...
  return {
    ...
    avisosPrecio,
    // Una factura donde TODAS las líneas van en cero no es una venta. Se marca para que el
    // endpoint la rechace; una línea suelta en cero (bonificación, muestra) sí es legítima.
    todoEnCero: lineas.length > 0 && lineasEnCero === lineas.length,
  };
}
