...
const productos = {
  ...
  columnas: [
    ...
    // Tratamiento e impuestos van en columnas separadas: son cosas distintas y meterlas en
    // una sola casilla fue justo lo que confundía IVA con impuesto al consumo.
    { key: "tratamientoIva", header: "Tratamiento IVA", width: 16 },
    { key: "impuestos", header: "Impuestos", width: 26 },
  ...
  async exportar(uid) {
    ...
    return rows.map((p) => ({
      ...
      // Códigos del catálogo, no porcentajes: son la clave estable para reimportar.
      impuestos: (p.impuestos || []).map((pi) => pi.impuesto?.codigo).filter(Boolean).join(", "),
  ...
  async importar(uid, filas) {
    ...
    // Catálogo indexado por código, para resolver la columna "Impuestos" sin una consulta
    // por fila. La carga masiva es donde un error se multiplica por 500 filas sin que nadie
    // lo mire, así que aquí la validación es estricta: nada de valores por defecto.
    const catalogo = new Map(
      (
        await prisma.impuesto.findMany({
          where: { activo: true, OR: [{ usuarioId: null }, { usuarioId: uid }] },
        })
      ).map((i) => [i.codigo.toUpperCase(), i])
    );
    const tratamientosValidos = TRATAMIENTOS_IVA.map((t) => t.valor);
    ...
    for (let i = 0; i < filas.length; i++) {
      ...
      const tratamientoIva = (txt(f["Tratamiento IVA"]) || "gravado").toLowerCase();
      if (!tratamientosValidos.includes(tratamientoIva)) {
        errores.push({
          fila,
          mensaje: `Tratamiento "${tratamientoIva}" no válido. Opciones: ${tratamientosValidos.join(", ")}.`,
        });
        continue;
      }

      const codigosImp = txt(f["Impuestos"])
        .split(/[,;]/)
        .map((s) => s.trim().toUpperCase())
        .filter(Boolean);
      const desconocidos = codigosImp.filter((c) => !catalogo.has(c));
      if (desconocidos.length) {
        errores.push({
          fila,
          mensaje: `Impuesto(s) no reconocido(s): ${desconocidos.join(", ")}. Usa los códigos del catálogo (p. ej. IVA_19, INC_8).`,
        });
        continue;
      }
      if (tratamientoIva === "excluido" && codigosImp.length > 0) {
        errores.push({ fila, mensaje: "Un producto excluido no puede llevar impuestos." });
        continue;
      }
      if (tratamientoIva === "gravado" && codigosImp.length === 0) {
        errores.push({ fila, mensaje: "Un producto gravado debe declarar al menos un impuesto." });
        continue;
      }

      const impuestoIds = codigosImp.map((c) => catalogo.get(c).id);
      const data = {
        descripcion,
      ...
      const enlaces = { deleteMany: {}, create: impuestoIds.map((impuestoId) => ({ impuestoId })) };
      try {
        await prisma.producto.upsert({
          where: { usuarioId_codigo: { usuarioId: uid, codigo } },
          update: { ...data, impuestos: enlaces },
          create: { usuarioId: uid, codigo, ...data, impuestos: { create: enlaces.create } },
        });
        creados++;
      } catch {
