...
export async function generarComprobantePDF(comprobante, opciones = {}) {
  ...
  // `orientation: "landscape"` es imprescindible: con la orientación vertical por defecto,
  // jsPDF normaliza el formato poniendo el lado mayor como alto, así que [595, 421] se
  // convertía en una página de 421 de ancho y el contenido se salía por la derecha.
  const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: [ANCHO, ALTO] });

  // Se leen del documento y no de las constantes: si la orientación vuelve a jugar una mala
  // pasada, el dibujo se adapta en vez de desbordarse en silencio.
  const W = doc.internal.pageSize.getWidth();
  const H = doc.internal.pageSize.getHeight();

  dibujar(doc, comprobante, opciones, 0, W, H);

  if (opciones.copias === 2) {
    doc.addPage([ANCHO, ALTO], "landscape");
    dibujar(doc, comprobante, { ...opciones, copia: true }, 0, W, H);
  }
