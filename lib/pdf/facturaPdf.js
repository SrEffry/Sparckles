...
export async function generarFacturaPDF(f, opciones = {}) {
  ...
  // ---- Encabezado: emisor ----
  // El logo se lee de la configuración ACTUAL, no del snapshot: es marca, no dato fiscal.
  // Si no hay logo o está roto, `dibujarLogo` devuelve 0 y el texto arranca en el margen.
  const anchoLogo = dibujarLogo(doc, opciones.logo, { x: M, y: 40, maxAncho: 110, maxAlto: 46 });
  const xTexto = M + (anchoLogo ? anchoLogo + 14 : 0);

  doc.setFont("helvetica", "bold").setFontSize(15).setTextColor(...BRAND);
  doc.text(f.emisorRazonSocial || "Emisor", xTexto, 54);
  ...
  emisorLineas.forEach((t, i) => doc.text(t, xTexto, 70 + i * 12));
