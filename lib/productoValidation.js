...
export function normalizarProducto(body) {
  ...
  // La tarifa debe ser una de la tabla. Antes se guardaba lo que llegara, con "0%" por
  // defecto: un valor que el cálculo no reconociera se liquidaba silenciosamente como 0.
  const tarifaIva = T(body.tarifaIva);
  if (!tarifaIva) {
    errors.push("Seleccione la tarifa de IVA del producto.");
  } else if (!VALORES_TARIFA.includes(tarifaIva)) {
    errors.push(
      `La tarifa de IVA "${tarifaIva}" no es válida. Opciones: ${VALORES_TARIFA.join(", ")}.`
    );
  }
