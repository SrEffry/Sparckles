 ...
 * Documento soporte.
 *
 * UNA SOLA LÍNEA VINCULANTE POR OPERACIÓN: la del hecho que ocurrió PRIMERO. La retención se
 * practica en el pago o abono en cuenta, lo que suceda antes (Art. 365 y ss. E.T.), y solo se
 * certifica una vez.
 *
 *   · Soporte normal (se causa y después se paga) → es el primer hecho: VINCULANTE. La
 *     política de causación no le aplica, porque su pago no puede reconocerla por separado.
 *   · Soporte generado desde un comprobante de egreso → el pago ya ocurrió y ya escribió su
 *     línea vinculante: esta va INFORMATIVA. Sin esta distinción, el certificado del Art. 381
 *     sumaba las dos y le certificaba al proveedor el doble de lo que se le retuvo, mientras
 *     el formulario 350 declaraba la mitad.
 */
export async function registrarRetencionesDeSoporte(tx, usuarioId, soporte) {
  const lineas = [];
  ...
  return registrarRetenciones(tx, {
    ...
    lineas,
    vinculante: !soporte.generadoDesdeComprobanteId,
  });
}
