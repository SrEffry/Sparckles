...
export async function retencionesParaCertificado(
  ...
  // Documentos de ReteFuente sin concepto VÁLIDO: el certificado no puede decir "por algo se
  // le retuvo el 4%". Se comprueba contra la tabla y no solo que el campo tenga algo: un
  // texto libre como "honorarios" es truthy y pasaba el guardián sin ser un concepto oficial.
  // Se devuelven con su origen, porque no todos se arreglan en el mismo módulo.
  const sinConcepto = [
    ...new Map(
      certificables
        .filter((l) => l.tipo === "retefuente" && !buscarConcepto(l.conceptoCodigo))
        .map((l) => [l.docRef, { docRef: l.docRef, origen: l.origen }])
    ).values(),
  ];

  // ReteIVA sin el valor de la operación gravada: el art. 1.6.1.12.13 del Decreto 1625 lo
  // exige aparte del IVA generado, y sin él el certificado saldría con "$0,00" en una columna
  // obligatoria — aritméticamente imposible y por tanto inservible para el tercero.
  const sinOperacion = [
    ...new Map(
      certificables
        .filter((l) => l.tipo === "reteiva" && !(Number(l.baseOperacion) > 0))
        .map((l) => [l.docRef, { docRef: l.docRef, origen: l.origen }])
    ).values(),
  ];
